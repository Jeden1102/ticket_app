<template>
  <div class="client__review" v-if="review && review.attributes">
    ...
    <div class="client__review__content">
      ...
      <h5 class="desc" v-html="review.attributes.description"></h5>
      <div class="event" v-if="review.attributes.event.data">
        <p class="event_title">
          {{ review.attributes.event.data.attributes.Title }}
        </p>
        <p class="event_location">
          {{ review.attributes.event.data.attributes.Location }}
        </p>
        <p class="event_data">
          {{ formatDate(review.attributes.event.data.attributes.Date) }}
        </p>
      </div>
    </div>
  </div>
